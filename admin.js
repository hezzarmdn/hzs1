...
function loginAdmin() {
  ...
  if(key==='admin123'){ // ganti sesuai password lu
    document.getElementById('adminPanel').classList.remove('hidden');
  } else alert('Password salah!');
}

// Reset uptime pakai server
async function resetUptime(){
  const { data, error } = await sb.rpc('get_server_time'); // rpc server now
  const serverTime = Date.now(); // fallback lokal jika rpc belum ada
  const { error: err } = await sb.from('uptime').update({ start_time: serverTime }).eq('id',1);
